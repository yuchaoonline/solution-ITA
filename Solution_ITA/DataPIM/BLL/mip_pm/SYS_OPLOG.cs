...
namespace Mis.BLL.ptgl
{
    ...
    /// <summary>
    /// 业务逻辑类 SYS_OPLOG 的摘要说明。
    /// </summary>
    public class SYS_OPLOG
    {
        private static readonly ISYS_OPLOG dal = DataAccess.CreateSYS_OPLOG();
        ...
        public gtled.Model.Post.SYS_OPLOG ToLocalData(DataRow row)
        {
            gtled.Model.Post.SYS_OPLOG data = new gtled.Model.Post.SYS_OPLOG();
            data.ID = int.Parse(row["ID"].ToString());
            data.USERID = (string)row["USERID"].ToString();
            data.USERNAME = (string)row["USERNAME"].ToString();
            data.OPTYPE = (string)row["OPTYPE"].ToString();
            data.OPTIME = (string)row["OPTIME"].ToString();
        ...
        public gtled.Model.Post.SYS_OPLOG[] ToLocalData(DataTable table)
        {
            gtled.Model.Post.SYS_OPLOG[] userobjs = new gtled.Model.Post.SYS_OPLOG[table.Rows.Count];
        ...
        public gtled.Model.Post.SYS_OPLOG NullToSpace(gtled.Model.Post.SYS_OPLOG item)
        {
            if (item.USERID == null)
                item.USERID = "";
