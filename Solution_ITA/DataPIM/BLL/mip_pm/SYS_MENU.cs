...
namespace Mis.BLL.ptgl
{
    ...
    /// <summary>
    /// 业务逻辑类 SYS_MENU 的摘要说明。
    /// </summary>
    public class SYS_MENU
    {
        private static readonly ISYS_MENU dal = DataAccess.CreateSYS_MENU();
        public SYS_MENU()
        { }
        #region  成员方法
        ...
        public Djdw.Model.Post.SYS_MENU ToLocalData(DataRow row)
        {
            Djdw.Model.Post.SYS_MENU data = new Djdw.Model.Post.SYS_MENU();
            data.ID = int.Parse(row["ID"].ToString());
            data.SYSNAME = (string)row["SYSNAME"].ToString();
            data.MOUDLENAME = (string)row["MOUDLENAME"].ToString();
            data.SYSLEVEL = int.Parse(row["SYSLEVEL"].ToString());
            data.PARENT = (string)row["PARENT"].ToString();
            data.URL = (string)row["URL"].ToString();
            data.DESCRIPTION = (string)row["DESCRIPTION"].ToString();
            data.IMG = (string)row["IMG"].ToString();
            data.VERSION = int.Parse(row["VERSION"].ToString());
            return data;
        }
        ...
        public Djdw.Model.Post.SYS_MENU[] ToLocalData(DataTable table)
        {
            Djdw.Model.Post.SYS_MENU[] userobjs = new Djdw.Model.Post.SYS_MENU[table.Rows.Count];
        ...
        public Djdw.Model.Post.SYS_MENU NullToSpace(Djdw.Model.Post.SYS_MENU item)
        {
            if (item.SYSNAME == null)
                item.SYSNAME = "";
            if (item.MOUDLENAME == null)
                item.MOUDLENAME = "";
            if (item.PARENT == null)
                item.PARENT = "";
            if (item.URL == null)
