...
namespace DBUtility
{
    ...
    {
        public static string connectionString = PubConstant.GetConnectionString("JtOracleConnection");

        public static IDbConnection connectionobj = null;
        ...
        public static int GetMaxIDBySeq(string FieldName, string TableName)
        {
            //select SEQ_YX.nextval from dual
            string strsql = "select SEQ" + TableName + ".nextval  from dual";
            object obj = GetSingle(strsql);
            if (obj == null)
            {
                return -1;
            ...
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                ...
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        {
                            DictionaryEntry myDE = (DictionaryEntry)SQLStringList[i];
                            string cmdText = myDE.Key.ToString();
                            OracleParameter[] cmdParms = (OracleParameter[])myDE.Value;
                            PrepareCommand(cmd, conn, trans, cmdText, cmdParms);
                            int val = cmd.ExecuteNonQuery();
